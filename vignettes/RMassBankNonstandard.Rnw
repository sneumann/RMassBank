...
data, recalibration can be deactivated. To do this, the \code{recalibrator}
entry in the settings must be set to \code{recalibrate.identity}. This can be
...
<<fig=TRUE,echo=FALSE>>=
currentRecal <- w@rc
hist2d(currentRecal$x, currentRecal$y, col=c("white", heat.colors(12)))
x <- seq(range(currentRecal$x)[[1]], range(currentRecal$x)[[2]])
predY <- predict(currentRecal, newdata=x)
lines(x, predY)	
@

Some example peaks to show the effect of recalibration:
<<>>=
w@recalibratedSpecs[[1]]$parentPeak[30:33,]
w@recalibratedSpecs[[1]]$peaks[[1]][15:18,]
@

Now reprocess the recalibration step with \code{recalibration.identity}:

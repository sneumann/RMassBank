parseMassBank <- function(Files){
	...
	for(i in 1:length(Files)){
		...
		##This is not in RMassBank yet, so I don't know whether I should include it?
		...
		numpeak <- grep('PK$NUM_PEAK:',record, fixed = TRUE)
			...
			if(length(PKannotationStart) > 0 && ReadAnnotations == TRUE){
	...
	print("Read alle Files")
	flush.console()
	return(mb)
}
recordsToRMassBank <- function(Files){
	mb <- new("mbWorkspace")
	mb@compiled_ok <- list()
	for(i in 1:length(Files)){
		fileConnection <- file(normalizePath(Files[i]))
